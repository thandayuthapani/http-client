...
func TestHttpDo(t *testing.T) {
	...
	registryClient := &go_sc_client.RegistryClient{}
	err := registryClient.Initialize(
		go_sc_client.Options{
			Addrs: []string{"127.0.0.1:30100"},
		})
	assert.NoError(t, err)

	url := fmt.Sprintf("http://127.0.0.1:30100%s", fmt.Sprintf("%s%s", go_sc_client.MS_API_PATH, go_sc_client.MicroservicePath))

	resp, err := uc.HttpDo("GET", url, nil, nil)
...
func TestHttpDoURLClient(t *testing.T) {
	...
	resp, err := c2.HttpDo("GET", url, nil, nil)
	assert.Nil(t, resp)
	assert.Error(t, err)
